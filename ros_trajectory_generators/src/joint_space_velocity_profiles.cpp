...
cubicPolynomial::cubicPolynomial(const KDL::Chain& kdl_chain)
{
   ros_tools::useJointVelLims(use_vel_limits);
   ros_tools::useJointAccLims(use_acc_limits);
   
   if(use_vel_limits) ros_tools::getJointVelLimsFromURDF(kdl_chain, max_vel);
   if(use_acc_limits) ros_tools::getJointAccLims(kdl_chain, max_acc);
   if(!use_vel_limits && !use_acc_limits) ROS_ERROR_STREAM("Can not create cubic polynomial velocity profile. \n"
      "Either \"use_joint_vel_lims\" or \"use_joint_acc_lims\" must be true.");
...
{ 
   ...
   else ROS_ERROR_STREAM("Can not create cubic polynomial velocity profile. Either \"use_joint_vel_lims\" \n"
				"or \"use_joint_acc_lims\" must be true.");
...
quinticPolynomial::quinticPolynomial(const KDL::Chain& kdl_chain)
{
   ros_tools::useJointVelLims(use_vel_limits);
   ros_tools::useJointAccLims(use_acc_limits);
   ...
   if(use_vel_limits) ros_tools::getJointVelLimsFromURDF(kdl_chain, max_vel);
   if(use_acc_limits) ros_tools::getJointAccLims(kdl_chain, max_acc);
   if(!use_vel_limits && !use_acc_limits) ROS_ERROR_STREAM("Can not create quintic polynomial velocity profile. \n"
      "Either \"use_joint_vel_lims\" or \"use_joint_acc_lims\" must be true.");
...
void quinticPolynomial::solve(const KDL::JntArray& qi, const KDL::JntArray& qf, double& duration)
{
   ...
   else ROS_ERROR_STREAM("Can not create quintic polynomial velocity profile. \n"
				"Either \"use_joint_vel_lims\" or \"use_joint_acc_lims\" must be true.");
...
bangBang::bangBang(const KDL::Chain& kdl_chain)
{
   ros_tools::useJointVelLims(use_vel_limits);
   ros_tools::useJointAccLims(use_acc_limits);
   ...
   if(use_vel_limits) ros_tools::getJointVelLimsFromURDF(kdl_chain, max_vel);
   if(use_acc_limits) ros_tools::getJointAccLims(kdl_chain, max_acc);
   if(!use_vel_limits && !use_acc_limits) ROS_ERROR_STREAM("Can not create bang bang velocity profile. \n"
      "Either \"use_joint_vel_lims\" or \"use_joint_acc_lims\" must be true.");
...
void bangBang::solve(const KDL::JntArray& qi, const KDL::JntArray& qf, double& duration)
{
   ...
   else ROS_ERROR_STREAM("Can not create bang bang velocity profile. \n"
      "Either \"use_joint_vel_lims\" or \"use_joint_acc_lims\" must be true.");
...
trapezoidal::trapezoidal(const KDL::Chain& kdl_chain)
{
   ros_tools::getJointVelLimsFromURDF(kdl_chain, max_vel);
   ros_tools::getJointAccLims(kdl_chain, max_acc);
...
bool getJointSpaceVelProfile(std::string vel_prof_name, const KDL::Chain& kdl_chain, std::unique_ptr<joint_space_vel_prof::velocityProfile>& vel_prof)
{
   if(vel_prof_name == "trapezoidal"){
      vel_prof.reset(new joint_space_vel_prof::trapezoidal(kdl_chain));
      ROS_ERROR_STREAM("Creating trapezoidal joint space velocity profile.");
      return true;
   }
   else if(vel_prof_name == "bang_bang"){
      vel_prof.reset(new joint_space_vel_prof::bangBang(kdl_chain));
      ROS_ERROR_STREAM("Creating bang_bang joint space velocity profile.");
      return true;
   }
   else if(vel_prof_name == "quintic_polynomial"){
      vel_prof.reset(new joint_space_vel_prof::quinticPolynomial(kdl_chain));
      ROS_ERROR_STREAM("Creating quintic_polynomial joint space velocity profile.");
      return true;
   }
   else if(vel_prof_name == "cubic_polynomial"){
      vel_prof.reset(new joint_space_vel_prof::cubicPolynomial(kdl_chain));
      ROS_ERROR_STREAM("Creating cubic_polynomial joint space velocity profile.");
      return true;
   }
   else{ROS_ERROR_STREAM("function argument " << vel_prof_name << " does not match a velocity profile."
      " Options are \"trapezoidal\", \"bang_bang\", \"quintic_polynomial\", \"cubic_polynomial\"");
      return false;
   }
}
